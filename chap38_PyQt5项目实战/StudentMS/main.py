...
class Ui_MainWindow(QMainWindow):
    ...
    def setupUi(self, MainWindow):
        ...
        self.statusbar.showMessage(
            "当前登录用户：" + service.userName + " | 登录时间：" + time + "  | 版权所有：IceRan", 0)
        # 为基础设置惨淡中的QAction绑定triggered信号
    ...
    def openSet(self, m):
        if m.text() == "年级设置":
            self.m = grade.Ui_MainWindow()  # 实例化年级设置窗体
            self.m.show()
        elif m.text() == "班级设置":
            self.m = classes.Ui_MainWindow()  # 实例化班级设置窗体
            self.m.show()
